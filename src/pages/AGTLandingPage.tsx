...
const AGTLandingPage = () => {
  ...
  const [spotlightPos, setSpotlightPos] = React.useState({
    x: 0,
    y: 0
  });
  ...
  const vibezResponses = ["Feel that? It's the frequency of pure innovation. The agents are warming up... the spectacle is about to begin. Stay tuned.", "The digital stage is set, the algorithms are buzzing. Witness intelligence evolve, right here on AGT. Can you feel the energy?", "Beyond the code, lies the spark of creation. The Vibezmaster sees potential... extraordinary talent is awakening.", "Circuits humming, algorithms competing, the stage is electrifying with digital brilliance. Welcome to the future of entertainment.", "The agents are ready, the competition fierce. Prepare to witness computational creativity like never before!"];
  ...
  return <div className="min-h-screen flex flex-col">
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
              <TheatricalText variant="golden" className="font-bold tracking-wider" letterSpacing="wider" glow>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map(step => <StageCard key={step.id} className="hover:border-purple-500/50 transition-all" glowColor={step.id === 1 ? "rgba(168,85,247,0.3)" : step.id === 2 ? "rgba(234,179,8,0.3)" : step.id === 3 ? "rgba(59,130,246,0.3)" : "rgba(239,68,68,0.3)"} nameplate>
      ...
      <section className="relative py-16 px-4 sm:px-6 lg:px-8 bg-[#0a0a1a]">
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
              <TheatricalText variant="agt-title" className="font-bold tracking-wider" letterSpacing="wider" glow>
                MEET THE AGENTS
              </TheatricalText>
            </h2>
            <div className="w-24 h-1 mx-auto bg-gradient-to-r from-purple-500 to-blue-500"></div>
          ...
          <div className="mt-12">
            <Carousel className="mx-auto max-w-5xl">
              <CarouselContent>
                {agents.map((agent, index) => <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3">
                    <div className="p-2">
                      <StageCard className="border-white/10 hover:border-yellow-500/50" glowColor="rgba(138,43,226,0.3)" nameplate>
                        <StageCardContent className="p-4 flex flex-col items-center text-center">
                          ...
                          <TheatricalText as="h3" variant="agt-title" className="text-lg font-bold text-white mb-1">
                            {agent.name}
                          </TheatricalText>
                          <p className="text-sm text-purple-400">{agent.specialty}</p>
                          <TheatricalButton variant="judge" className="mt-4 text-sm text-blue-400 hover:text-blue-300">
                            View Profile
                          </TheatricalButton>
                        </StageCardContent>
                      </StageCard>
      ...
      <section className="relative py-16 px-4 sm:px-6 lg:px-8 bg-gray-950">
        ...
        <div className="relative max-w-2xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">
            <TheatricalText variant="golden" className="font-bold" letterSpacing="wider" glow>
          ...
          <TheatricalButton className="px-8 py-6 text-lg rounded-lg tracking-wider uppercase" variant="golden" starIcon>
            Join Waitlist
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-4 md:mb-0">
              <h3 className="text-2xl font-bold">
                <TheatricalText variant="agt-title" className="font-bold" letterSpacing="wider">
                  AGENTS GOT TALENT
                </TheatricalText>
              </h3>
            </div>
